...
class TwoFactorAuthenticationStatus
{
    ...
    public function handle($request, Closure $next, $status = 'enabled')
    {
        ...
        if ($status === 'enabled' && $request->is('backoffice/*') && ! config('boilerplate.access.user.admin_requires_2fa')) {
            return $next($request);
        }
        ...
        if (
            ...
            // return redirect()->route('frontend.auth.account.2fa.create')->withFlashDanger(__('Two-factor Authentication must be :status to view this page.', ['status' => $status])); //gua yang comment
